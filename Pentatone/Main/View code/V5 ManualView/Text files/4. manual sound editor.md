 ...
 ** CHECKED PAGE 1-3
 ...
 This sets the carrier oscillator frequency to a multiple of the base frequency of the note that is being played. Changing this will affect the pitch of the sound directly, and set it to the corresponding harmonic of the base note.
 ...
 This provides fine-tuning adjustment to the modulator oscillator frequency, adding a fractional values (0-1) to the coarse multiplier setting. For example, if the coarse multiplier is set to 2 and the fine multiplier is 0.5, the total modulator multiplier will be 2.5. This allows for more subtle timbral variations and can be used to create inharmonic or detuned sounds.
 ...
 This sets the time it takes for the sound to reach full volume after a note is triggered. Short attack times (0-5ms) produce immediate, percussive sounds, while longer attack times create gradual swells. The range extends from 0ms to 5000ms, allowing for everything from sharp strikes to slow, pad-like fades.
 ...
 This sets the cutoff frequency of the lowpass filter, determining which frequencies are allowed to pass through. Frequencies below the cutoff pass through largely unaffected, while frequencies above the cutoff are progressively attenuated. Lower settings create darker, more muffled tones, while higher settings yield brighter sounds. The range spans from 55Hz to 14080Hz, covering the full audible spectrum. The parameter range is chosen so that it is easy to set the filter frequency to exact note values in standard tuning. This allows the resonant frequency of the filter to correspond to the pitch of the note being played. To achieve a filter frequency that is identical to the note being played, set the value to 440 (or a multiple thereof) and set key track to filter (see page 3) to 1.00.
 ...
 This makes the filter cutoff frequency respond to the pitch of notes being played. Higher values cause higher notes to open the filter more, creating a more natural response where brighter notes sound brighter. This helps maintain consistent timbral balance across the keyboard range and is particularly useful for emulating acoustic instruments. Set this to 1.00 to have the filter frequency follow the note frequency exactly - this is particularly useful at higher resonance settings, when the filter frequency is set to a maultiple of 440 Hz.
 
 7) Key track to voice lfo rate amount
 This causes the Voice LFO speed to increase with higher notes. When set to higher values, notes played higher on the keyboard will have faster vibrato or modulation effects, while lower notes have slower modulation. This can create more natural-sounding expression, particularly when the voice LFO is used for vibrato.
